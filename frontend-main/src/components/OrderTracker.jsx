  ...
  const fetchFilters = useCallback(async () => {
    try {
      const [usersRes, productsRes] = await Promise.all([
        axios
          .get(`${BASE_URL}/api/users`, { headers: getAuthHeaders() })
          .catch(() => ({ data: [] })),
        axios
          .get(`${BASE_URL}/products`, { headers: getAuthHeaders() })
          .catch(() => ({ data: [] })),
      ]);
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (
          ...
          activeAlerts.length > prevFraudCountRef.current
        ) {
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to fetch order tracker data",
        background: "#1e293b",
        color: "#f1f5f9",
      });
    } finally {
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col">
      ...
      <div className="bg-dark-900 border-b border-dark-700 px-4 sm:px-6 py-4 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-dark-400 text-xs">
              {tableData.length} order items tracked
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleToggleFraudSound}
            className="p-2 bg-dark-800 rounded-xl hover:bg-dark-700"
            title={fraudSoundEnabled ? "Mute alerts" : "Unmute alerts"}
          >
            {fraudSoundEnabled ? (
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  onClick={() => {
                    ...
                    if (fraudAudioRef.current) fraudAudioRef.current.pause();
                  }}
                  className="px-3 py-1 bg-red-500/30 text-red-300 rounded-lg text-sm hover:bg-red-500/50"
                >
                  Dismiss
                </button>
              </div>
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {fraudAlerts
                  ...
                  .map((alert, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between bg-red-950/50 rounded-lg px-3 py-2 text-sm"
                    >
                      <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {Object.entries(NETWORK_CONFIG).map(([key, cfg]) => {
            ...
            return (
              <div
                key={key}
                className={`${cfg.bg} border ${cfg.border} rounded-2xl p-5 transition-all hover:scale-[1.02]`}
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">{cfg.icon}</span>
                    <h3 className={`font-bold text-lg ${cfg.text}`}>
                      {cfg.label}
                    </h3>
                  </div>
                  <div
                    className={`px-3 py-1 rounded-full text-xs font-bold ${cfg.bg} ${cfg.text} border ${cfg.border}`}
                  >
                    {data.count} orders
                  </div>
                </div>
                <p className="text-2xl font-bold text-white">
                  {formatGHS(data.total)}
                </p>
        ...
        <div className="bg-dark-800/50 border border-dark-700 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <div className="relative" ref={agentDropdownRef}>
              ...
              >
                <span
                  className={selectedAgent ? "text-white" : "text-dark-500"}
                >
                  {selectedAgent
            ...
            {/* Product dropdown */}
            <div>
              <label className="text-dark-500 text-xs mb-1 block">
                Product
              </label>
              <select
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {paginated.map((row, i) => {
                      const isFraud = fraudAlerts.some(
                        (f) =>
                          f.orderId === row.orderId &&
                          f.itemId === row.itemId &&
                          !resolvedIds.includes(`${f.orderId}-${f.itemId}`),
                      );
                      return (
                        <tr
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-dark-400 whitespace-nowrap text-xs">
                ...
                <div className="flex items-center justify-between px-4 py-3 border-t border-dark-700">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => setPage(1)}
                      disabled={page === 1}
                      className="px-2 py-1 bg-dark-700 rounded text-dark-300 text-xs disabled:opacity-30"
                    >
                      First
                    </button>
                    <button
