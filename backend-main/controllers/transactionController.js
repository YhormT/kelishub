...
const getAdminBalanceSheetData = async (req, res) => {
  try {
    ...
    // 1. Total Revenue (Sales) — completed line items × product price
    ...
    const orderCount = completedItems.length;
    ...
    const previousBalanceQuery = await prisma.$queryRaw`
      SELECT COALESCE(SUM(t.balance), 0)::double precision AS "previousBalance"
      FROM "Transaction" t
      INNER JOIN (
